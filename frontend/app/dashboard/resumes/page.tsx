...
export default function DashboardResumesPage() {
  ...
  const [previewData, setPreviewData]   = useState<ResumePreview | null>(null);
  ...
  async function handlePreview(id: string) {
    ...
    try {
      setPreviewData(await getResumePreview(id));
    } catch (e) {
  ...
  async function handleDelete(resume: ResumeListItem) {
    ...
    try {
      ...
      if (previewId === resume.id) { setPreviewId(null); setPreviewData(null); }
      await loadResumes();
    } catch (e) {
    ...
    <div className="space-y-6">
      ...
      <Panel className="overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Saved Resumes</p>
        ...
        {!loading && resumes.length === 0 && !fetchError && (
          <div className="mx-6 mb-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center">
            <p className="text-base font-semibold text-slate-900">No resumes yet</p>
            <p className="mt-2 text-sm text-slate-500">Upload a PDF or DOCX file above to get started.</p>
          </div>
        )}
                            ...
                            {previewData && (
                              <div className="space-y-3">
                                ...
                                <pre className="max-h-64 overflow-y-auto whitespace-pre-wrap rounded-xl border border-slate-200 bg-white p-4 font-mono text-xs leading-5 text-slate-700">
                                  {previewData.raw_text_preview || "No text extracted yet."}
                                </pre>
                              </div>
                            )}
                          </td>

  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-8 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-brand-700 mb-1">{t("campaignsPage.eyebrow")}</p>
          <h1 className="text-xl font-bold text-slate-800">{t("campaignsPage.title")}</h1>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 space-y-3">
          ...
          <p className="text-sm text-slate-500">{t("campaignsPage.noCampaigns")}</p>
          <Link href="/dashboard/smart-send/letter" className="inline-block bg-brand-800 text-white rounded-xl px-5 py-2.5 text-sm font-semibold hover:bg-brand-700">
            {t("campaignsPage.startCampaignBtn")}
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
              ...
              <div key={campaign.id} className="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ${STATUS_STYLES[campaign.status]}`}>
                    {t(`campaigns.status.${campaign.status}` as Parameters<typeof t>[0])}
                  </span>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-xs text-slate-500 mb-1">
                    <span>{campaign.total_sent} {t("campaigns.status.sent")}</span>
                    <span>{campaign.total_contacts} {t("campaignsPage.totalLabel")}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400 mt-1">
                    ...
                    {campaign.total_failed > 0 && <span className="text-rose-500">{campaign.total_failed} {t("campaigns.status.failed")}</span>}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-slate-400">
                  <span>{t("campaignsPage.dailyLimit", { limit: campaign.daily_limit })}</span>
                  {campaign.last_sent_at && (
                    <span>{t("campaignsPage.lastSent", { date: new Date(campaign.last_sent_at).toLocaleDateString("ar-SA") })}</span>
                  )}
                  {campaign.estimated_days_remaining > 0 && campaign.status === "active" && (
                    <span>{t("campaignsPage.remainingDays", { days: campaign.estimated_days_remaining, unit: campaign.estimated_days_remaining === 1 ? t("campaignsPage.day") : t("campaignsPage.days") })}</span>
                  )}
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  {campaign.status === "active" && (
                    <button onClick={() => void handlePause(campaign.id)} disabled={isBusy} className="text-xs border border-amber-200 text-amber-700 rounded-lg px-3 py-1.5 hover:bg-amber-50 disabled:opacity-50">
                      {isBusy ? t("campaignsPage.actioning") : t("campaignsPage.pauseBtn")}
                    </button>
                  )}
                  {campaign.status === "paused" && (
                    <button onClick={() => void handleResume(campaign.id)} disabled={isBusy} className="text-xs border border-teal-light text-teal rounded-lg px-3 py-1.5 hover:bg-teal-light/20 disabled:opacity-50">
                      {isBusy ? t("campaignsPage.actioning") : t("campaignsPage.resumeBtn")}
                    </button>
                  )}
                  {(campaign.status === "error" || campaign.total_failed > 0) && (
                    <button onClick={() => void handleRetry(campaign.id)} disabled={isBusy} className="text-xs border border-brand-200 text-brand-700 rounded-lg px-3 py-1.5 hover:bg-brand-50 disabled:opacity-50">
                      {isBusy ? t("campaignsPage.actioning") : t("campaignsPage.retryBtn")}
                    </button>
                  )}
                </div>
              </div>

    ...
    <div className="space-y-6">
      ...
      <Panel className="p-6 md:p-8">
        ...
        <h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-900">
          Select a resume and request the AI report
        </h2>
        ...
        <div className="mt-6 space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700" htmlFor="jd-input">
              ...
              <span className="font-normal text-slate-400">(optional — AI will pick a suitable role if empty)</span>
            </label>
            <textarea
              id="jd-input"
              rows={5}
          ...
          >
            {pageState === "streaming" ? "Generating report…" : "Request Analysis"}
          </button>
        </div>
      </Panel>
        ...
        <Panel className="p-6 md:p-8">
          <div className="mb-4 flex items-center justify-between">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
              {pageState === "streaming" ? "Generating…" : pageState === "done" ? "Report" : "Error"}
            </p>
            {pageState === "streaming" && (
      ...
      <Panel className="overflow-hidden">
        ...
        {/* Inline view of a selected historical report */}
        {viewReport && viewReport.report_text && (
          <div className="border-t border-slate-100 px-6 py-6">
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
              Report — {viewReport.resume_title} — {formatDate(viewReport.created_at)}
            </p>
